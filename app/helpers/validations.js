...
export const step2schema = Yup.object({
  serviceTime: Yup.object({
    mode: Yup.string()
      .oneOf(["oneTime", "regular"])
      .required("Mode is required"),

    oneTimeDate: Yup.string().when("mode", {
      is: "oneTime",
      then: (schema) =>
        schema
          .required("One-time date is required")
          .test("is-valid-format", "Date must be in YYYY-MM-DD format", (val) =>
            isValidDateString(val)
          )
          .test(
            "min-date",
            `Date must be at least ${minOneTimeDate.toLocaleDateString()}`,
            (val) => {
              if (!isValidDateString(val)) return false;
              const d = parseDateString(val);
              const m = new Date(minOneTimeDate);
              m.setHours(0, 0, 0, 0);
  ...
  specialServicesSection: Yup.object({
    specialServicesSwitch: Yup.boolean(),
    specialServices: Yup.array().when("specialServicesSwitch", {
      is: true,
      then: (schema) =>
        schema
          .required("Please select at least one special service")
          .min(1, "Please select at least one special service"),
      otherwise: (schema) => schema.notRequired(),
    }),
  }),
});

export const updateBookedServiceSchema = Yup.object({
  service: Yup.object({
      ...
      then: (schema) =>
        schema.shape({
          genderOption: Yup.string()
            .oneOf(["any", "select"])
            .required("Gender option is required"),
          anyQty: Yup.string().when("genderOption", {
            is: "any",
            then: (s) =>
              s
          ...
          femaleRequired: Yup.boolean(),
          femaleQty: Yup.string().when(["genderOption", "femaleRequired"], {
            is: (g, f) => g === "select" && f === true,
            then: (s) =>
              s
                .required("Female Expert Qty is required")
                .matches(/^[0-9]+$/, "Must be a number"),
            otherwise: (s) => s.nullable(),
          }),
          maleRequired: Yup.boolean(),
          maleQty: Yup.string().when(["genderOption", "maleRequired"], {
            is: (g, m) => g === "select" && m === true,
            then: (s) =>
              s
                .required("Male Expert Qty is required")
                .matches(/^[0-9]+$/, "Must be a number"),
            otherwise: (s) => s.nullable(),
          }),
          hours: Yup.string().required("Working hours are required"),
        }),
      otherwise: (schema) => schema.strip(), // or nullable()
    }),
  }),
  propertyForService: Yup.array()
    .of(
      Yup.object({
    ...
    .required("Select at least one property"),
  serviceTime: Yup.object({
    mode: Yup.string()
      .oneOf(["oneTime", "regular"])
      .required("Mode is required"),
